...
export default function ShopByPet() {
  return (
    <section className="max-w-container-max mx-auto px-4 md:px-margin-desktop py-6">
      <h2 className="text-sm font-bold text-on-surface mb-3">Shop by Pet</h2>
      <div className="grid grid-cols-5 gap-3">
        {PETS.map(({ name, href, image }) => (
          <Link key={name} href={href} className="group cursor-pointer">
            <div className="aspect-square rounded-xl overflow-hidden bg-surface-container border border-outline-variant/30 mb-2 relative">
            ...
            <p className="text-xs font-medium text-center group-hover:text-primary transition-colors">{name}</p>
          </Link>
        ))}
      </div>
    </section>
