          ...
          {isInitial ? (
            <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/20 p-4 space-y-3">
              <div className="flex justify-between items-center mb-2">
                <div className="w-24 h-4 rounded-xl bg-on-surface/10 animate-shimmer" />
                <div className="flex gap-2">
                  <div className="w-7 h-7 rounded-xl bg-on-surface/10 animate-shimmer" />
                  <div className="w-7 h-7 rounded-xl bg-on-surface/10 animate-shimmer" />
                </div>
              </div>
              <div className="grid grid-cols-7 gap-1">
                {Array.from({ length: 35 }).map((_, i) => (
                  <div key={i} className="aspect-square rounded-xl bg-on-surface/10 animate-shimmer" />
                ))}
              </div>
            </div>
          ) : (
              ...
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-surface-container-low">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="h-3 w-32 rounded bg-on-surface/10 animate-shimmer" />
                    <div className="h-2.5 w-24 rounded bg-on-surface/10 animate-shimmer" />
                  </div>
                  <div className="w-16 h-5 rounded-full bg-on-surface/10 animate-shimmer" />
                </div>
              ))}
            </div>
          ) : (
